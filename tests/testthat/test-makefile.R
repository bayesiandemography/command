...
test_that("'makefile' works with has dir_make, no name_make", {
  ...
  writeLines("cmd_assign(.data = 'data/mydata.csv', use_log = FALSE, .out = 'out/cleaned.rds')",
             con = file_path)
  suppressMessages(ans_obtained <- makefile(files = "src", dir_make = dir_tmp))
  ans_expected <- paste0("\n",
                         ".PHONY: all\n",
                         "all:\n",
                         "\n\n",
...
test_that("'makefile' works with no dir_make, has name_make", {
  ...
  writeLines("cmd_assign(.data = 'data/mydata.csv', use_log = FALSE, .out = 'out/cleaned.rds')",
             con = "src/script.R")
  suppressMessages(ans_obtained <- makefile(files = "src", name_make = "Make"))
  ans_expected <- paste0("\n",
                         ".PHONY: all\n",
                         "all:\n",
                         "\n\n",
...
test_that("'makefile' works with no dir_make, no files", {
  dir_curr <- getwd()
  dir_tmp <- tempfile()
  if (file.exists(dir_tmp))
    unlink(dir_tmp, recursive = TRUE)
