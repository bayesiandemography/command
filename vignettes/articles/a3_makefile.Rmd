...
Functions `extract_make()` and `makefile()` help with writing a Makefile to control a data analysis workflow. `extract_make()` and `makefile()` do much of the work, but some extra hand coding is also necessary.
...
`extract_make()` extracts the call to `cmd_assign()` from an R file, and turns it into a Makefile rule. For instance, the call
...
In the call to `cmd_assign()` above, the names `.cleaned_data` and `.out` start with a dot, and the name `rlm_method` does not. `extract_make()` treats dotted name-value pairs differently fron non-dotted ones. A dotted name-value pair is assumed to refer to an unnamed file path passed at the command line, such as `out/cleaned_data.rds` or `out/model.rds`. A non-dotted pair is assumed to refer to a named argument, such as `--rlm_method=M`.

The easiest way to use `extract_make()` is to call it from the R console, and then cut-and-paste the results into Makefile.
...
In the examples above, all the command line arguments were filenames. Command line arguments can also, however, be values. The following command, for instance, includes a named argument called `n_iter` with value `100`. 
```{bash, eval = FALSE}
Rscript myfile.R --n_iter=100 myoutput.rds
```

The corresponding call to `cmd_assign()` will look something like this:
```{R, eval = FALSE}
...
- [Modeular Workflows for Data Analysis](https://bayesiandemography.github.io/command/articles/workflow.html)
