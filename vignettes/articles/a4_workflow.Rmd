...
Makefiles have an enormous range of features. The extra power tends, however, to come at the expense of readability. Moreover, most data analysis projects have a sufficiently simple structure that the extra power is not really needed. If 'VPATH directives' are added to a Makefile, for instance, then directory names can be omitted from file paths, so that `model.R` can be used instead of `src/model.R` [@gnu2024make, Section 4.5]. Omitting directory names makes Makefiles more concise, but it also makes them harder to understand for readers who are not Makefile experts.
