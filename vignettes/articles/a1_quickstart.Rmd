...
dir_tree(regexp = paste(nms_articles, 
                        "Makefile|workflow.sh|report.Rmd", 
                         sep = "|"),	
         invert = TRUE)
```
...
The third command in `workflow.sh` calls the `quarto` application directly, rather than through the R function [quarto_render](https://quarto-dev.github.io/quarto-r/reference/quarto_render.html).
...
Makefiles take some time to master, but they have important advantages, discussed in [Modular Workflows for Data Analysis](https://bayesiandemography.github.io/command/articles/workflow.html).
