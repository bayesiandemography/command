...
This article shows how to get started with package **command**. It should ideally be read in combination with the article [A Workflow for Data Analysis](https://bayesiandemography.github.io/command/articles/workflow.html).

**command** contains a single function, `cmd_assign()`, which assigns objects to the working environment. `cmd_assign()` can be called in two ways:
...
Both code snippets add two objects to the working environment, with names `"obj1"` and `"obj2"`, and values `"orange"` and `1`.
...
```{r, echo = FALSE}
dir_tree(regexp = "quickstart|workflow|Makefile|workflow.sh|report.Rmd",
         invert = TRUE)
```
...
3. **Assign** the resulting values to the working environment.
...
Finally, in the **assign** step, `cmd_assign()` puts the values into the working environment.
...
We illustrate with a simple example. We need two new files. The first file, called `report.qmd`, creates a with two figures. It contains the following code.
```{r, echo = FALSE}
...
The third line of `workflow.sh` contains an `"-e"` after the `"Rscript"`. The `"-e"` option tells Rscript to work with a single R expression (which follows immediately after the `"-e"`), rather than a file.
...
```{r, echo = FALSE}
dir_tree(regexp = "quickstart|workflow.Rmd|Makefile",
         invert = TRUE,
	 recurse = 0)
```
...
Makefiles take some time to master, but they have important advantages, which we discuss in [A Workflow for Data Analysis](https://bayesiandemography.github.io/command/articles/workflow.html).
...
One example is  [commandArgs()](https://search.r-project.org/CRAN/refmans/R.utils/html/commandArgs.html) in package [R.utils](https://cran.r-project.org/web/packages/R.utils/index.html), which is a more user-friendly version of the base R function `commandArgs()`, with many additional processing options.

Another is package [docopt](https://cran.r-project.org/web/packages/docopt/index.html), which can be used to construct an interface to a script, including processing command line arguments.

`cmd_assign()` is more specialised than `commandArgs()` or **docopt**. It focuses specifically on the task of processing command line arguments as part of a data analysis workflow. This allows it to have a simpler interface, with more automation.


```{r, include = FALSE}
files_to_delete <- c("airmiles.csv",
		     "fig_smoothed_5.png",
		     "fig_smoothed_8.png",
		     "fig_smoothed_10.png",
		     "fig_smoothed.R",
		     ...
		     "report_files",
		     "workflow.sh")
path <- path(getwd(), files_to_delete)
file_delete(path)
```
